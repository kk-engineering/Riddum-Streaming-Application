//
//  ProfileTrackPlayerViewController.swift
//  RiddumProject
//
...
class ProfileTrackPlayerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        trackLabel.text = userTracksSavedToLibrary[counter].title
        artistLabel.text = userTracksSavedToLibrary[counter].artist
    }
    
    // Retrieves cover art of track.
    func retrieveCoverArt() {
        let currentTrackImage = userTracksSavedToLibrary[counter].imageUrl
    ...
    func retrieveMetadata() {
        let currentTrack = userTracksSavedToLibrary[counter].url
    ...
    @IBAction func prev(_ sender: Any) {
        if playerFilled == true && counter != 0 {
            playThis(thisTrack: userTracksSavedToLibrary[counter-1].url)
            counter -= 1
            trackLabel.text = userTracksSavedToLibrary[counter].title
    ...
    @IBAction func next(_ sender: Any) {
        if playerFilled == true && counter < userTracksSavedToLibrary.count-1 {
            playThis(thisTrack: userTracksSavedToLibrary[counter+1].url)
            counter += 1
            trackLabel.text = userTracksSavedToLibrary[counter].title
